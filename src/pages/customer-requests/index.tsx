...
type ColumnType = ColumnDef<CustomerRequestInterface, unknown>;

interface CustomerRequestListPageProps {
...
export function CustomerRequestListPage(props: CustomerRequestListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getCustomerRequests({
        ...
        order: params.order,
        searchTermKeys: ['request_period.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CustomerRequestInterface>>(
    () => `/customer-requests?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CustomerRequestInterface) => {
    if (hasAccess('customer_request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/customer-requests/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'request_period', header: 'Request Period', accessorKey: 'request_period' },
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/customer-requests/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="customer_request" />
        </Flex>
